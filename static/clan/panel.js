      ...
      axios.post("./api/", {
        ...
        if (res.data.code == 0) {
          ...
          if (res.data.notice) {
            ...
            //     duration: 60000,
            // });
            thisvue.$q.notify('(' + (new Date()).toLocaleTimeString('chinese', { hour12: false }) + ') ' + res.data.notice)
          }
    ...
    callapi: function (payload) {
      ...
      axios.post("./api/", payload).then(function (res) {
        if (res.data.code == 0) {
          ...
          thisvue.$q.notify(res.data.message, '数据错误')
        }
      }).catch(function (error) {
        // thisvue.$alert(error, '数据错误');
        thisvue.$q.notify(error, '数据错误')
      });
    },
