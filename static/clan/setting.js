  ...
  mounted() {
    ...
    axios.post('./api/', {
      ...
      if (res.data.code == 0) {
        ...
        thisvue.$q.notify(res.data.message, '加载数据失败');
      }
  ...
  methods: {
    submit: function (event) {
      ...
      axios.post('./api/', {
        ...
        if (res.data.code == 0) {
          thisvue.$q.notify('设置成功');
        } else {
          thisvue.$q.notify(res.data.message, '保存设置失败');
        }
      }).catch(function (error) {
        thisvue.$q.notify(error, '保存设置失败');
      });
    },
    ...
    call_api: function (payload) {
      ...
      axios.post('./api/', payload).then(function (res) {
        if (res.data.code == 0) {
          ...
          thisvue.$q.notify(res.data.message, '失败');
        }
      }).catch(function (error) {
        thisvue.$q.notify(error, '失败');
      });
    },
